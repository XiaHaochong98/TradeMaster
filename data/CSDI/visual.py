...
parser.add_argument("--dataind", type=int, default=0)
...
#plt.pause(2)
path3 = "./visual/" + args.dataset + "/" + args.tic + str(args.testmissingratio)
os.makedirs(path3, exist_ok=True)
plt.savefig(path3 + "/point chart" + str(args.dataind) + ".png")
plt.show()
plt.close()
...
title_name = "Candlestick for " + args.tic
path4 = path3 + "/candlestick chart" + str(args.dataind) + ".png"
add_plot=[mpf.make_addplot(test_impute, ylabel="imputed price($)", type='candle', panel=1)]
mpf.plot(test_gt,type='candle',addplot=add_plot, title=title_name, ylabel="raw price($)", style="binance", main_panel=0, panel_ratios=(1,1), savefig = path4)
