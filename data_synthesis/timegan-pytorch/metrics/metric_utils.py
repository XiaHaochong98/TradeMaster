...
def post_hoc_discriminator(ori_data, generated_data):
    ...
    for epoch in logger:
        ...
        logger.set_description(f"batchnumL {len(train_dataloader)}, Epoch: {epoch},loss: {running_loss/len(train_dataloader):.4f}, real_loss: {running_real_loss/len(train_dataloader):.4f}, fake_loss: {running_fake_loss/len(train_dataloader):.4f}")
