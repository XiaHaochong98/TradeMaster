...
# def normalization_with_history(data, history_data):
#     # use a specific feature of the histrocial data to normalize some features in the data
#     normalizor=['low']
#     normalized_features=['open','high','low','close']
#
#     scaler = MinMaxScaler()
#     # fit on the normalizor of history data
#     normalization_data=history_data[normalizor].values.reshape(-1, 1)
#     scaler.fit(normalization_data)
#     # transform each column in normalized_features of history data and data respectively with the scaler
#     normalized_history_data=history_data.copy()
#     normalized_data=data.copy()
#     for feat in normalized_features:
#         normalized_history_data[feat]=scaler.transform(history_data[feat].values.reshape(-1,1))
#         normalized_data[feat]=scaler.transform(data[feat].values.reshape(-1,1))
#     #record the mean and std of the normalizor
#     mean=scaler.mean_[0]
#     std=np.sqrt(scaler.var_[0])
#
#
#     # normalize the reparmetrized features O_minus_L,C_minus_L,H_minus_maxOC by casting them to have a mean of 1
#
#
#
#     return mean,std,normalized_data,normalized_history_data
...
def normalization(data_sample,history_sample):
    ...
    # print('hist',history_sample.shape)
    # print('shape',history_sample['low'].values.reshape(-1,1).shape)
...
def normalization_inverse(data_sample,scaler_dict,scaler_order,feature_order):
    ...
    for sample in range(data_sample.shape[0]):
        # print(low_scaler[sample].inverse_transform(data_sample[sample,:,low_index].reshape(-1, 1)).shape)
        # print(data_sample[sample,:,low_index].shape)
        # re
        data_sample[sample,:,low_index]=low_scaler[sample].inverse_transform(data_sample[sample,:,low_index].reshape(-1, 1)).reshape(-1)
        # inverse the normalization of 'O_minus_L','C_minus_L','H_minus_maxOC'
        # print(data_sample[sample,:,O_minus_L_index].shape,O_minus_L_scaler[sample].shape)
        ...
        data_sample[sample,:,H_minus_maxOC_index]=data_sample[sample,:,H_minus_maxOC_index]/H_minus_maxOC_scaler[sample]
...
def reparameterization_inverse(data, original_feature_order, feature_order):
    ...
    # print(low_i,open_i,close_i,high_i)
    # print('low',data[:,:,low_i])
    # print('open',open)
    # print('close',close)
    # print('high',high)
...
def conditional_data_sample_worker(data, seq_len, features, history_length, full_data, differential_features=True):
    ...
    for interval_index,interval in enumerate(intervals):
        ...
        for i in range(0, len(data_seg) - seq_len):
            ...
            normalized_data_sample,normalized_history_sample,low_scaler,O_minus_L_scaler,C_minus_L_scaler,H_minus_maxOC_scaler=normalization(data_sample,history_sample)
...
def conditional_dataset_preprocessing(data_file_path, tics, args, tic_tokenizer, dynamics_tokenizer, features, history_length):
    ...
    #
    # if not os.path.exists('./data/segs/'):
    #     os.makedirs('./data/segs/')
    # #clean segs folder
    # for file in os.listdir('./data/segs/'):
    #     os.remove('./data/segs/'+file)
    ...
    for tic in tqdm(tics):
        ...
        for j in range(regime_num):
            ...
            # data_seg.to_csv(f'./data/segs/data_seg_{tic}_{j}.csv')
            # data_seg = pd.read_csv(f'./data/segs/data_seg_{tic}_{j}.csv')
            data_samples,historcial_samples,time,low_scaler_vec,O_minus_L_scaler_vec,C_minus_L_scaler_vec,H_minus_maxOC_scaler_vec,last_hist_vec,original_data_samples,original_history_samples= conditional_data_sample_worker(data_seg, args.max_seq_len, features, history_length, data, args.differential_features) # list of dataframes dim: (n,seq_len,features) time: (n,1)
            #broadcast tic and dynamics token and scaler to each data slice
            #TODO check if slice is correct
            ...
            # scaler_vector=np.array(scaler_vector)
...
def conditional_rescale_data(ori_data, scaler, differential_features, last_hist_value, scaler_order, original_feature_order):
    ...
    # print(f'normalization_inverse data preview data {data[:2, :10, :]}')
    ...
    # print(f'undifferential normalization_inverse data preview data {data[:2, :10, :]}')
