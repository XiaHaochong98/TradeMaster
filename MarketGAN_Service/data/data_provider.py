...
def data_provider(data_file_path, tics, args, tic_tokenizer, dynamics_tokenizer, features,history_length):
    ...
    for tic in tqdm(tics):
        ...
        for j in range(regime_num):
            ...
            data_samples,historcial_samples,last_hist_vec = data_sample_worker(data_seg, args.max_seq_len, features, history_length, data) # list of dataframes dim: (n,seq_len,features) time: (n,1)
            ...
            # data_sample_list_vec.append(data_samples)
            #
            #
            # Data_sample = np.concatenate(data_sample_list_vec, axis=0)  # dim: (n,seq_len,features)
            # save data sample to file
            # print(f'size of data sample of tic {tic} and dynamic {j} is ',Data_sample.shape)
            # np.save(f'{args.model_path}/{tic}_{j}.npy',Data_sample)
            # print('save data sample to file: ',f'{args.model_path}/{tic}_{j}.npy')
