...
from metrics.metric_utils import (
    feature_prediction, one_step_ahead_prediction, post_hoc_discriminator,feature_constraint_evaluaton
)
from metrics.visualization import *
from models.conditional_timegan import Conditional_TimeGAN
# from models.utils import timegan_trainer, timegan_generator,conditional_timegan_trainer,conditional_timegan_generator
...
class MarketGAN(object):
    ...
    def init(self,args,lognow=False):
        ##############################################
    ...
    def preprocess(self, data_path, args):
        ...
        if True or args.is_train==True:
            ...
            # if args.conditional:
            #     print('using conditional data, waiting for preprocessing')
            #     X, T ,D , L,scaler=conditional_dataset_preprocessing(data_path,tics,args,tic_tokenizer,dynamics_tokenizer,features)
            #     args.padding_value=-1
            # else:
            ...
            # rescale verify
            # already verified
            # rescled_X = rescale_data(X, scaler)
            # #compare OX and rescled_X
            # print('OX',OX)
            # print('rescled_X',rescled_X)
            # # make diff between OX and rescled_X
            # diff=OX-rescled_X
            # print('diff',diff)
            # print('rescale verify')
            ...
            train_X, test_X, train_time, test_time, train_D, test_D, train_L, test_L, train_scaler, test_scaler,train_H, test_H,train_Last_h,test_Last_h,OX_train,OX_test,OH_train,OH_test=train_test_split_by_conditions(args, dynamics_tokenizer, tic_tokenizer, X, T, D, L, scaler, H, Last_h, OX, OH)
            # train_X, test_X, train_time, test_time, train_D, test_D, train_L, test_L, train_scaler, test_scaler,train_H, test_H,train_Last_h,test_Last_h,OX_train,OX_test,OH_train,OH_test = train_test_split(
            #     X, T, D, L, scaler,H,Last_h,OX,OH, test_size=1-args.train_rate, random_state=args.seed
            # )
            # print(test_Last_h.shape)
            ...
            if args.conditional:
                test_history_rescaled = conditional_rescale_data(test_H, test_scaler, args.differential_features, test_Last_h_history, args.scaler_order, original_feature_order=args.feature)
                test_data_rescaled = conditional_rescale_data(test_X, test_scaler, args.differential_features, test_Last_h_data,
            ...
            # testifiy if the rescaled data is the same as the original data
            # if the element of OX is not none
            try:
                # rescale
                # print('test rescale data shape and preview:', test_data_rescaled.shape, test_data_rescaled[:2])
                # print('OX_test shape and preview:', OX_test.shape, OX_test[:2])
                ...
                test_X_encode=data_preprocessing_for_condition_discrimination(test_history_rescaled, test_data_rescaled, test_Last_h_history , args.feature, args.differential_features)
                ...
                # print('OX_test shape and preview:', OX_test.shape, OX_test[:1,:5,:])
                # print('Last_h shape and preview:', test_Last_h.shape, test_Last_h[:1,:])
                # print('test_history_rescaled shape and preview:', test_history_rescaled.shape, test_history_rescaled[:1,-2:,:])
                # diff between test_X_encode and test_X
                diff=test_X_encode-test_X
                # print('diff between test_X_encode and test_X shape and preview:', diff.shape, diff[:2,:,:])
        ...
        # if args.conditional:
        #     train_X, test_X, train_time, test_time, train_D, test_D, train_L, test_L, train_scaler, test_scaler = train_test_split(
        #         X, T, D, L, scaler, test_size=args.train_rate, random_state=args.seed
        #     )
        # else:
        #     train_X, test_X, train_time, test_time = train_test_split(
    ...
    def train(self,args, train_X, test_X, train_time, test_time, train_D, test_D, train_L, test_L, train_scaler, test_scaler,train_H, test_H,train_Last_h,test_Last_h):
        #########################
    ...
    def inference(self,  args, model, X, T, D,L,H, scaler, Last_h,dynamic,ticker,feature,sample_indexs,noise_multiplier=1,dynamic_multipler=1,plot_num=10,sample_number=64,exceed_generation_length=0):
        ...
        dynamic_prossed=dynamic*dynamic_multipler
        print(f'noise multiplier {noise_multiplier}, dynamic multiplier {dynamic_multipler}, dynamic_prossed preview {dynamic_prossed[0]}')
        ...
        for i,sample_seed in enumerate(seed_list):
            ...
            if noisy_dynamic:
                ...
                dynamic_prossed_noised=dynamic_prossed
            ...
            generated_X_sample = conditional_timegan_generator(model=model, T=T, args=args,dynamics=dynamic_prossed_noised,labels=ticker,history=history_prossed_noised,noise_multiplier=noise_multiplier,seed=sample_seed)

            # rescale the generated data to the original scale


            generated_data_rescaled_sample = conditional_rescale_data(generated_X_sample, scaler, args.differential_features, Last_h_data, args.scaler_order, original_feature_order=args.feature)
        ...
        generated_data_rescaled=np.mean(generated_samples,axis=0)
        ...
        # print(f'Generated data rescaled preview:\n{generated_data_rescaled.shape, generated_data_rescaled[:1, :5, :]}\n')
        # print(f'real data rescaled preview:\n{data_rescaled.shape, data_rescaled[:1, :5, :]}\n')
        # print(f'history rescaled preview:\n{history_rescaled.shape, history_rescaled[:1, :5, :]}\n')
        # print(f"Model Generation Runtime: {(end_time - start_time) / 60} mins\n")

        # log the dynamic vector into a readable string dynamic_info
        # the dynamic should be a vector of (sample_num,dymanic_dim)
        # if the elements of all the samples are the same, we only need to log the first sample as the dynamic_info
        # if the elements of all the samples are different, we let dynamic_info to be 'mixed'
        dynamic_info = ''
        if np.all(dynamic == dynamic[0]):
            dynamic_info = str(dynamic[0])
        else:
            dynamic_info = 'mixed'
        # log the ticker vector into a readable string ticker_info
        # the ticker should be s vector of (sample_num,dymanic_dim)
        # if the elements of all the samples are the same, we only need to log the first sample as the ticker_info
        # if the elements of all the samples are different, we let ticker_info to be 'mixed'
        ticker_info = ''
        if np.all(ticker == ticker[0]):
            ticker_info = str(ticker[0])
        else:
            ticker_info = 'mixed'
        #########################
        # Save train and generated data for visualization
        #########################
        generated_data_path= args.model_path+f'/inference_data/{dynamic_multipler}_D_{noise_multiplier}_N'
        # make inference_data_path if not exist
        if not os.path.exists(generated_data_path):
            os.makedirs(generated_data_path)
        # Save generated data
        np.save(f"{generated_data_path}/generated_data_rescaled_{dynamic_info}.npy", generated_data_rescaled)
        np.save(f"{generated_data_path}/data_rescaled.npy", data_rescaled)
        np.save(f"{generated_data_path}/history_rescaled.npy", history_rescaled)
        # plot samples of generated data with history
            # sample a random index
        for i,sample_index in enumerate(sample_indexs):
            generated_sample= generated_data_rescaled[i]
            real_sample=data_rescaled[i]
            history_sample=history_rescaled[i]
            data=np.concatenate((history_sample,generated_sample),axis=0)
            # print('generated data shape',data.shape)
            plot_OHLCcharts(data,feature,generated_data_path,fig_suffix=f'index({sample_index})_{dynamic_prossed[i]}_generated')
            # plot the real data with history
            # print('real data shape', data.shape)
            #if generated_data_path + f'/OHLC_{fig_suffix}.png' does not exist, plot the chart
            if not os.path.exists(generated_data_path + f'/OHLC_index({sample_index})_{dynamic_prossed[i]}_real.png'):
                data=np.concatenate((history_sample,real_sample),axis=0)
                plot_OHLCcharts(data,feature,generated_data_path,fig_suffix=f'index({sample_index})_real')
            if i>=plot_num:
                break


        return args, model,generated_data_rescaled,data_rescaled,history_rescaled



    def generation(self,  args, model, train_X, test_X, train_time, test_time, train_D, test_D, train_L, test_L, train_scaler,
            test_scaler, train_H, test_H,train_Last_h,test_Last_h,exceed_generation_length=0):
        # we use the trained dynamics and label to test generation for now
        #TODO: test generation with different dynamics and label
        # Log start time
        evaluation_test=False
        print(f'Using randoms noise as generated data: {evaluation_test}')
        start_time = time.time()
        ...
        # print(f"Generated data as test preview:\n{generated_test_X.shape, generated_test_X[:2, -10:, :2]}\n")
        # print(f"Generated data as train preview:\n{generated_train_X.shape, generated_train_X[:2, -10:, :2]}\n")
        ...
        # print(f'111 train data preview: {train_X.shape, train_X[:2, :10, :]}\n')
        # print(f'111 generated data preview: {generated_train_X.shape, generated_train_X[:2, :10, :]}\n')

        # print('rescale generated')
        if args.conditional:
            generated_data_as_train_rescaled = conditional_rescale_data(generated_train_X, train_scaler, args.differential_features, train_Last_h_data, args.scaler_order, original_feature_order=args.feature)
            generated_data_as_test_rescaled = conditional_rescale_data(generated_test_X, test_scaler, args.differential_features, test_Last_h_data, args.scaler_order, original_feature_order=args.feature)
            # print('rescale train')
            train_data_rescaled = conditional_rescale_data(train_X, train_scaler, args.differential_features, train_Last_h_data, args.scaler_order, original_feature_order=args.feature)
            test_data_rescaled = conditional_rescale_data(test_X, test_scaler, args.differential_features, test_Last_h_data, args.scaler_order, original_feature_order=args.feature)
            train_history_rescaled = conditional_rescale_data(train_H, train_scaler, args.differential_features, train_Last_h_history, args.scaler_order, original_feature_order=args.feature)
            test_history_rescaled = conditional_rescale_data(test_H, test_scaler, args.differential_features, test_Last_h_history, args.scaler_order, original_feature_order=args.feature)
        else:
    ...
    def evaluate(self, args, model, dynamics_book, dynamics_tokenizer, label_book, tic_tokenizer,
        ...
        if plot:
            ...
            generated_data_as_train_rescaled_norm, train_data_rescaled_norm = normalize_data(generated_data_as_train_rescaled, train_data_rescaled)
            visualization(generated_data_as_train_rescaled, train_data_rescaled, analysis="tsne", args=args,
                          fig_suffix=f'{model_name}_train')
            # visualization(generated_data_as_train_rescaled_norm, train_data_rescaled_norm, analysis="tsne", args=args,
            #               fig_suffix=f'{model_name}_train_norm')
            generated_data_as_test_rescaled_norm,test_data_rescaled_norm=normalize_data(generated_data_as_test_rescaled, test_data_rescaled)
            # visualization(generated_data_as_test_rescaled, test_data_rescaled, analysis="tsne", args=args,
            #               fig_suffix=f'{model_name}_test')
            # visualization(generated_data_as_test_rescaled_norm, test_data_rescaled_norm, analysis="tsne", args=args,
            #               fig_suffix=f'{model_name}_test_norm')
            for d in dynamics_book:
                ...
                # data_list.append(real_d_data_norm)
                # generated_data_list.append(fake_d_data_norm)
                ...
                # visualization(fake_d_data_norm, real_d_data_norm, analysis="tsne", args=args,fig_suffix=f'{model_name}_test_dynamics_{d}_normalized')
                # visualization(fake_d_data, real_d_data, analysis="tsne", args=args,fig_suffix=f'{model_name}_test_dynamics_{d}')
            # # visualization_by_dynamic(data_list[0],data_list[1],data_list[2],args=args,fig_suffix=f'{model_name}_dynamics_plot')
            # # visualization_by_dynamic(data_list[0], data_list[1], data_list[2], generated_data_list[0],
            # #                          generated_data_list[1], generated_data_list[2], args=args,
            # #                          fig_suffix=f'{model_name}_dynamics_plot')

            # # visualization_by_dynamic(data_list[0],data_list[1],data_list[2],generated_data_list[0],generated_data_list[1],generated_data_list[2],args=args,fig_suffix=f'{model_name}_dynamics_plot')
            # # visualization_by_dynamic(norm_data_list[0], norm_data_list[1], norm_data_list[2], norm_generated_data_list[0],
            # #                          norm_generated_data_list[1], norm_generated_data_list[2], args=args,
            # #                          fig_suffix=f'{model_name}_dynamics_plot_with_real_norm')
            # # visualization_by_dynamic(data_list[0],data_list[1],data_list[2],generated_data_list[0],generated_data_list[1],generated_data_list[2],args=args,fig_suffix=f'{model_name}_dynamics_with_real_plot')
            # # visualization_by_dynamic_solo(data_list[0],data_list[1],data_list[2],args=args,fig_suffix=f'Real_dynamics_plot')
            # # visualization_by_dynamic_solo(generated_data_list[0],generated_data_list[1],generated_data_list[2],args=args,fig_suffix=f'{model_name}_dynamics_plot')
            if args.exp == 'DJ30_V2_RNN_T_supervisor':
            ...
            visualization_by_dynamic_solo(norm_generated_data_list[0], norm_generated_data_list[1], norm_generated_data_list[2], args=args,
            ...
            #     # print(f"Running PCA plot of generated data and real data for dynamics {d} on test set...")
            #     # visualization(fake_d_data, real_d_data, analysis="pca", args=args,fig_suffix=f'test_dynamics_{d}')
            # # labels
            ...
            for l in label_book_random_picked:
                ...
                # real_l_data=train_data_rescaled[l_index]
                real_l_data_t = test_data_rescaled[l_index_t]
                # # get the data of the l_token index in the generated_data
                # fake_l_data=generated_data_as_train_rescaled[l_index]
                fake_l_data_t = generated_data_as_train_rescaled[l_index_t]
                # # visualize the data
                # print(f"Running TSNE plot of generated data and real data for label {l} {model_name} on train set...")
                # visualization(fake_l_data, real_l_data, analysis="tsne", args=args,fig_suffix=f'{model_name}_train_label_{l}')
                # print(f"Running PCA plot of generated data and real data for label {l} on train set...")
                # visualization(fake_l_data, real_l_data, analysis="pca", args=args,fig_suffix=f'{model_name}_train_label_{l}')
                # do the same thing for the test set
                # find the index of the l_token in the test_L
                # get all the index where test_L[:i]==l_token
                # l_index=np.where((test_L==l_token).all(axis=1))[0]
                # print(f'label data size of {l} is {len(l_index)} out of {test_data_rescaled.shape[0]}')
                # get the data of the l_token index
                # real_l_data=test_data_rescaled[l_index]
                # get the data of the l_token index in the generated_data
                # fake_l_data=generated_data_as_test_rescaled[l_index]
                # fake_l_data_norm, real_l_data_norm = normalize_data(fake_l_data, real_l_data)
                ...
                visualization(fake_l_data_t, real_l_data_t, analysis="tsne", args=args,fig_suffix=f'{model_name}_test_label_{l}')
                # visualization(fake_l_data_norm, real_l_data_norm, analysis="tsne", args=args,
                #               fig_suffix=f'{model_name}_test_label_{l}_nrom')
                # print(f"Running PCA plot of generated data and real data for label {l} on test set...")
                # visualization(fake_l_data, real_l_data, analysis="pca", args=args,fig_suffix=f'test_label_{l}')
            # # print("Running TSNE plot of generated data and test data...")
            # # visualization(generated_data_as_train_rescaled, train_data_rescaled,analysis="tsne",args=args)
            # # # print("Running PCA plot of generated data and test data...")
            # # visualization(generated_data_as_train_rescaled, train_data_rescaled,analysis="pca",args=args)
            return 0
        #########################
        # Evaluate the performance
        ################
        suffix='Final_verfication_discrimiantor'
        ...
        test_Last_h_data, test_Last_h_history = np.split(test_Last_h, 2, axis=1)
        #
        # # print(f'train_X preview:\n{train_X.shape, train_X[:2, :10, :2]}\n')
        # # print(f'generated_train_X preview:\n{generated_train_X.shape, generated_train_X[:2, :10, :2]}\n')
        #
        # # print(f'2222 Train data rescaled preview:\n{train_data_rescaled.shape, train_data_rescaled[:2, -10:, : ]}\n')
        # # print(f'2222 Generated_data_as_train_rescaled preview:{generated_data_as_train_rescaled.shape,generated_data_as_train_rescaled[:2,-10:,:]}')
        #
        #
        # # Normalize the rescaled data for the evaluation process
        # # train_data_rescaled_normalized,generated_data_as_train_rescaled_normalized=normalization_for_downstream_tasks(train_data_rescaled,generated_data_as_train_rescaled,train_history_rescaled)
        # # test_data_rescaled_normalized,generated_data_as_test_rescaled_normalized=normalization_for_downstream_tasks(test_data_rescaled,generated_data_as_test_rescaled,test_history_rescaled)
        # # print(f'downstream train_data_rescaled_normalized preview:{train_data_rescaled_normalized.shape,train_data_rescaled_normalized[:2,:10,:]}')
        # # print(f'downstream generated_data_as_train_rescaled_normalized preview:{generated_data_as_train_rescaled_normalized.shape,generated_data_as_train_rescaled_normalized[:2,:10,:]}')
        #
        ...
        # # # fake_step_ahead_pred_SMAPE_list = []
        # # # fake_step_ahead_pred_SMAPE_norm_list = []
        # # # fake_step_ahead_pred_SMAPE_avg_list = []
        # # # fake_step_ahead_pred_training_MSE_list = []
        ...
        #         #     (generated_data_as_train_rescaled_dynamic_tic, train_time_dynamic_tic),
        #         #     args,
        #         #     epoch=evaluation_epochs
        #         # )
        #         # discriminator_loss = discriminator_loss.item()
        #         # discrimination_loss_train = abs(discriminator_acc - 50)
        #         # discrimination_acc_train_list.append(discriminator_acc)
        #         # discrimination_loss_train_list.append(discrimination_loss_train)
        #         # # evaluation_results['discrimination_loss_train_dynamic_{}_tic_{}'.format(dynamic, tic)] = discrimination_loss_train
        #         # print('Discriminator acc on train: ' + str(discriminator_acc) + ", Discriminator loss on train " + str(
        #         #     discriminator_loss) + '\n')
        #         discriminator_acc, discriminator_loss = post_hoc_discriminator(
        ...
        #         # # evaluation_results['discrimination_loss_test_dynamic_{}_tic_{}'.format(dynamic, tic)] = discrimination_loss_test
        #         # print('Discriminator acc on test: ' + str(discriminator_acc) + ", Discriminator loss on test " + str(
        #         #     discriminator_loss) + '\n')
        #
        ...
        #         # # copy the train data
        #         # train_data_rescaled_dynamic_tic_downstream = copy.deepcopy(train_data_rescaled_dynamic_tic)
        #         # train_time_dynamic_tic_downstream = copy.deepcopy(train_time_dynamic_tic)
        #         # # copy the test data
        #         # test_data_rescaled_dynamic_tic_downstream = copy.deepcopy(test_data_rescaled_dynamic_tic)
        #         # test_time_dynamic_tic_downstream = copy.deepcopy(test_time_dynamic_tic)
        #         # print('train_data_rescaled_dynamic_tic_1',train_data_rescaled_dynamic_tic[0,:2,:2])
        #         # # use copied data as training set
        #         # real_step_ahead_pred_SMAPE, real_step_ahead_pred_SMAPE_norm, real_step_ahead_pred_training_MSE = one_step_ahead_prediction(
        #         #     (train_data_rescaled_dynamic_tic_downstream, train_time_dynamic_tic_downstream),
        #         #     (test_data_rescaled_dynamic_tic_downstream, test_time_dynamic_tic_downstream),
        #         #     args,
        ...
        #         # generated_data_as_train_rescaled_dynamic_tic_downstream = copy.deepcopy(generated_data_as_train_rescaled_dynamic_tic)
        #         #
        #         # # concatenate the generated data to the copied train data
        #         # # use copied data + generated data as training set
        ...
        #         #     (np.concatenate((train_data_rescaled_dynamic_tic_downstream, generated_data_as_train_rescaled_dynamic_tic_downstream), axis=0),
        #         #         np.concatenate((train_time_dynamic_tic_downstream, train_time_dynamic_tic_downstream), axis=0)),
        #         #     (test_data_rescaled_dynamic_tic_downstream, test_time_dynamic_tic_downstream),
        #         #     args,
        #         #     epochs=evaluation_epochs//2
        #         # )
        #         # real_fake_step_ahead_pred_SMAPE_avg=(real_fake_step_ahead_pred_SMAPE+real_fake_step_ahead_pred_SMAPE_norm)/2
            ...
            #     )
            #     real_step_ahead_pred_SMAPE_avg=(real_step_ahead_pred_SMAPE+real_step_ahead_pred_SMAPE_norm)/2
            #     print("TSTR:Running one step ahead prediction using generated data as training set...")
            #     # fake_step_ahead_pred_SMAPE, fake_step_ahead_pred_SMAPE_norm, fake_step_ahead_pred_training_MSE = one_step_ahead_prediction(
            #     #     (generated_data_as_train_rescaled_dynamic_tic, train_time_dynamic_tic),
            #     #     (test_data_rescaled_dynamic_tic, test_time_dynamic_tic),
            #     #     args,
            #     #     epochs=evaluation_epochs
            #     # )
            #     # fake_step_ahead_pred_SMAPE_avg=(fake_step_ahead_pred_SMAPE+fake_step_ahead_pred_SMAPE_norm)/2
            #     print('TSTR:Running one step ahead prediction using original+generated data as training set...')
            #     print('train_data_rescaled_dynamic_tic_2', train_data_rescaled_dynamic_tic[0, :2, :2])
            #     real_fake_step_ahead_pred_SMAPE, real_fake_step_ahead_pred_SMAPE_norm, real_fake_step_ahead_pred_training_MSE = one_step_ahead_prediction(
            #         (np.concatenate((train_data_rescaled_dynamic_tic, generated_data_as_train_rescaled_dynamic_tic), axis=0),
            ...
            #     real_fake_step_ahead_pred_SMAPE_avg=(real_fake_step_ahead_pred_SMAPE+real_fake_step_ahead_pred_SMAPE_norm)/2
                ...
                # # one_step_ahead_augmentation_score = np.round(
                # #     real_step_ahead_pred_SMAPE - min(real_fake_step_ahead_pred_SMAPE, fake_step_ahead_pred_SMAPE), 4)
                # # one_step_ahead_augmentation_score_norm = np.round(
                # #     real_step_ahead_pred_SMAPE_norm - min(real_fake_step_ahead_pred_SMAPE_norm,
                # #                                           fake_step_ahead_pred_SMAPE_norm), 4)
                # # one_step_ahead_augmentation_score_training_loss = np.round(
                # #     real_step_ahead_pred_training_MSE - min(real_fake_step_ahead_pred_training_MSE,
                # #                                             fake_step_ahead_pred_training_MSE), 4)
                # if args.exp == 'DJ30_V2_RNN_T_supervisor':
                ...
                # # fake_step_ahead_pred_SMAPE_list.append(fake_step_ahead_pred_SMAPE)
                # # fake_step_ahead_pred_SMAPE_norm_list.append(fake_step_ahead_pred_SMAPE_norm)
                # # fake_step_ahead_pred_SMAPE_avg_list.append(fake_step_ahead_pred_SMAPE_avg)
                # # fake_step_ahead_pred_training_MSE_list.append(fake_step_ahead_pred_training_MSE)
                ...
                # one_step_ahead_augmentation_score_list.append(one_step_ahead_augmentation_score)
                # one_step_ahead_augmentation_score_norm_list.append(one_step_ahead_augmentation_score_norm)

                # print('Generated data as training set-one step ahead prediction results(SMAPE):\n' +
                #       f'(1) Real training: {str(np.round(real_step_ahead_pred_SMAPE, 4)), str(np.round(real_step_ahead_pred_SMAPE_norm, 4)), str(np.round(real_step_ahead_pred_training_MSE, 4))}\n' +
                #       f'(2) Fake training: {str(np.round(fake_step_ahead_pred_SMAPE, 4)), str(np.round(fake_step_ahead_pred_SMAPE_norm, 4)), str(np.round(fake_step_ahead_pred_training_MSE, 4))}\n' +
                #       f'(3) Real+Fake training: {str(np.round(real_fake_step_ahead_pred_SMAPE, 4)), str(np.round(real_fake_step_ahead_pred_SMAPE_norm, 4)), str(np.round(real_fake_step_ahead_pred_training_MSE, 4))}\n' +
                #       f'(4) Improvements: {str(one_step_ahead_augmentation_score), str(one_step_ahead_augmentation_score_norm), str(one_step_ahead_augmentation_score_training_loss)}')
        # evaluation_results['real_step_ahead_pred_SMAPE'] = real_step_ahead_pred_SMAPE
        # evaluation_results['real_step_ahead_pred_SMAPE_norm'] = real_step_ahead_pred_SMAPE_norm
        # evaluation_results['real_step_ahead_pred_training_MSE'] = real_step_ahead_pred_training_MSE
        # evaluation_results['fake_step_ahead_pred_SMAPE'] = fake_step_ahead_pred_SMAPE
        # evaluation_results['fake_step_ahead_pred_SMAPE_norm'] = fake_step_ahead_pred_SMAPE_norm
        # evaluation_results['fake_step_ahead_pred_training_MSE'] = fake_step_ahead_pred_training_MSE
        # evaluation_results['real_fake_step_ahead_pred_SMAPE'] = real_fake_step_ahead_pred_SMAPE
        # evaluation_results['real_fake_step_ahead_pred_SMAPE_norm'] = real_fake_step_ahead_pred_SMAPE_norm
        # evaluation_results['real_fake_step_ahead_pred_training_MSE'] = real_fake_step_ahead_pred_training_MSE
        # evaluation_results['one_step_ahead_augmentation_score'] = one_step_ahead_augmentation_score
        # evaluation_results['one_step_ahead_augmentation_score_norm'] = one_step_ahead_augmentation_score_norm
        # evaluation_results[
        #     'one_step_ahead_augmentation_score_training_loss'] = one_step_ahead_augmentation_score_training_loss
        # evaluation_results_high_level[
        #     'TSTR:Next-step prediction (SMAPE) improvement'] = one_step_ahead_augmentation_score
        # evaluation_results_high_level[
        #     'TSTR:Next-step norm prediction (SMAPE) improvement '] = one_step_ahead_augmentation_score_norm
        # evaluation_results_high_level[
        #     'TSTR:Next-step prediction (training loss) improvement'] = one_step_ahead_augmentation_score_training_loss
        # store the average and std of the discrimination loss
        # if args.exp == 'DJ30_V2_RNN_T_supervisor':
    #     evaluation_results['real_step_ahead_pred_SMAPE'] = sum(real_step_ahead_pred_SMAPE_list) / len(real_step_ahead_pred_SMAPE_list)
    #     evaluation_results['real_step_ahead_pred_SMAPE_norm'] = sum(real_step_ahead_pred_SMAPE_norm_list) / len(real_step_ahead_pred_SMAPE_norm_list)
    #     evaluation_results['real_step_ahead_pred_training_MSE'] = sum(real_step_ahead_pred_training_MSE_list) / len(real_step_ahead_pred_training_MSE_list)
    #     evaluation_results['real_step_ahead_pred_SMAPE_avg'] = sum(real_step_ahead_pred_SMAPE_avg_list) / len(real_step_ahead_pred_SMAPE_avg_list)
    ...
    #     evaluation_results['real_step_ahead_pred_SMAPE_norm_detail'] = ','.join(real_step_ahead_pred_SMAPE_norm_detail_list)
    #     evaluation_results['real_step_ahead_pred_training_MSE_detail'] = ','.join(real_step_ahead_pred_training_MSE_detail_list)
    #     evaluation_results['real_step_ahead_pred_SMAPE_avg_detail'] = ','.join(real_step_ahead_pred_SMAPE_avg_detail_list)
    #     # evaluation_results['fake_step_ahead_pred_SMAPE'] = sum(fake_step_ahead_pred_SMAPE_list) / len(fake_step_ahead_pred_SMAPE_list)`
    #     # evaluation_results['fake_step_ahead_pred_SMAPE_norm'] = sum(fake_step_ahead_pred_SMAPE_norm_list) / len(fake_step_ahead_pred_SMAPE_norm_list)
    #     # evaluation_results['fake_step_ahead_pred_SMAPE_avg'] = sum(fake_step_ahead_pred_SMAPE_avg_list) / len(fake_step_ahead_pred_SMAPE_avg_list)
    #     # evaluation_results['fake_step_ahead_pred_training_MSE'] = sum(fake_step_ahead_pred_training_MSE_list) / len(fake_step_ahead_pred_training_MSE_list)
    #     evaluation_results['real_fake_step_ahead_pred_SMAPE'] = sum(real_fake_step_ahead_pred_SMAPE_list) / len(real_fake_step_ahead_pred_SMAPE_list)
    #     evaluation_results['real_fake_step_ahead_pred_SMAPE_norm'] = sum(real_fake_step_ahead_pred_SMAPE_norm_list) / len(real_fake_step_ahead_pred_SMAPE_norm_list)
    #     evaluation_results['real_fake_step_ahead_pred_SMAPE_avg'] = sum(real_fake_step_ahead_pred_SMAPE_avg_list) / len(real_fake_step_ahead_pred_SMAPE_avg_list)
    #     evaluation_results['real_fake_step_ahead_pred_training_MSE'] = sum(real_fake_step_ahead_pred_training_MSE_list) / len(real_fake_step_ahead_pred_training_MSE_list)
    #     # # store the std
    #     # evaluation_results['fake_step_ahead_pred_SMAPE_std'] = np.std(fake_step_ahead_pred_SMAPE_list)
    #     # evaluation_results['fake_step_ahead_pred_SMAPE_norm_std'] = np.std(fake_step_ahead_pred_SMAPE_norm_list)
    #     # evaluation_results['fake_step_ahead_pred_SMAPE_avg_std'] = np.std(fake_step_ahead_pred_SMAPE_avg_list)
    #     # evaluation_results['fake_step_ahead_pred_training_MSE_std'] = np.std(fake_step_ahead_pred_training_MSE_list)
    #     evaluation_results['real_fake_step_ahead_pred_SMAPE_std'] = np.std(real_fake_step_ahead_pred_SMAPE_list)
    #     evaluation_results['real_fake_step_ahead_pred_SMAPE_norm_std'] = np.std(real_fake_step_ahead_pred_SMAPE_norm_list)
    #     evaluation_results['real_fake_step_ahead_pred_training_MSE_std'] = np.std(real_fake_step_ahead_pred_training_MSE_list)
    #     evaluation_results['real_fake_step_ahead_pred_SMAPE_avg_std'] = np.std(real_fake_step_ahead_pred_SMAPE_avg_list)
        # # store the detail
        # fake_step_ahead_pred_SMAPE_detail_list = [str(i) for i in fake_step_ahead_pred_SMAPE_list]
        # fake_step_ahead_pred_SMAPE_norm_detail_list = [str(i) for i in fake_step_ahead_pred_SMAPE_norm_list]
        # fake_step_ahead_pred_training_MSE_detail_list = [str(i) for i in fake_step_ahead_pred_training_MSE_list]
        # fake_step_ahead_pred_SMAPE_avg_detail_list = [str(i) for i in fake_step_ahead_pred_SMAPE_avg_list]
        # evaluation_results['fake_step_ahead_pred_SMAPE_detail'] = ','.join(fake_step_ahead_pred_SMAPE_detail_list)
        # evaluation_results['fake_step_ahead_pred_SMAPE_norm_detail'] = ','.join(fake_step_ahead_pred_SMAPE_norm_detail_list)
        # evaluation_results['fake_step_ahead_pred_training_MSE_detail'] = ','.join(fake_step_ahead_pred_training_MSE_detail_list)
        # evaluation_results['fake_step_ahead_pred_SMAPE_avg_detail'] = ','.join(fake_step_ahead_pred_SMAPE_avg_detail_list)
        ...
        # real_fake_step_ahead_pred_training_MSE_detail_list = [str(i) for i in real_fake_step_ahead_pred_training_MSE_list]
        # real_fake_step_ahead_pred_SMAPE_avg_detail_list = [str(i) for i in real_fake_step_ahead_pred_SMAPE_avg_list]
        # evaluation_results['real_fake_step_ahead_pred_SMAPE_detail'] = ','.join(real_fake_step_ahead_pred_SMAPE_detail_list)
        # evaluation_results['real_fake_step_ahead_pred_SMAPE_norm_detail'] = ','.join(real_fake_step_ahead_pred_SMAPE_norm_detail_list)
        # evaluation_results['real_fake_step_ahead_pred_training_MSE_detail'] = ','.join(real_fake_step_ahead_pred_training_MSE_detail_list)
        # evaluation_results['real_fake_step_ahead_pred_SMAPE_avg_detail'] = ','.join(real_fake_step_ahead_pred_SMAPE_avg_detail_list)

        # # evaluation_results['discrimination_loss_train'] = sum(discrimination_loss_train_list) / len(discrimination_loss_train_list)
        # evaluation_results['discrimination_loss_test'] = sum(discrimination_loss_test_list) / len(discrimination_loss_test_list)
        # # calcluate the std of the discrimination loss
        # # evaluation_results['discrimination_loss_train_std'] = np.std(discrimination_loss_train_list)
        ...
        # # discrimination_acc_train_list = [str(i) for i in discrimination_acc_train_list]
        # discrimination_acc_test_list = [str(i) for i in discrimination_acc_test_list]
        # # evaluation_results['discrimination_acc_train'] = ','.join(discrimination_acc_train_list)
        ...
        # # evaluation_results_high_level['Discriminator loss_train']= evaluation_results['discrimination_loss_train']
        # # print('average discrimination_loss_train',evaluation_results['discrimination_loss_train'] )
        # print('average discrimination_loss_test', evaluation_results['discrimination_loss_test'])

        # # feature alignment on rescaled data
        # # real feature alignment
        # print(features)
        ...
        # print(f"Average of the percentage of the test data that doesn't satisfy the logic constraints: {round(avg_ratio,4)}\n")
        ...
        # if generated_test_X is None:
        #     print('cast the raw data to the processed format for dynamic prediction')
        #     generated_test_X = data_preprocessing_for_condition_discrimination(test_history_rescaled, generated_data_as_test_rescaled,
        #                                                                        args.history_length, args.feature,
        #                                                                        differential_features=args.differential_features)
        fake_d_acc, fake_l_acc, fake_d_loss, fake_l_loss = conditional_timegan_condition_prediction(model,
        ...
        # print("TSTR:Running one step ahead prediction using original data as training set...")
        # real_step_ahead_pred_SMAPE,real_step_ahead_pred_SMAPE_norm,real_step_ahead_pred_training_MSE= one_step_ahead_prediction(
        #     (train_data_rescaled, train_time),
        #     (test_data_rescaled, test_time),
        #     args,
        #     epochs=evaluation_epochs
        # )
        # print("TSTR:Running one step ahead prediction using generated data as training set...")
        # fake_step_ahead_pred_SMAPE,fake_step_ahead_pred_SMAPE_norm,fake_step_ahead_pred_training_MSE= one_step_ahead_prediction(
        #     (generated_data_as_train_rescaled, generated_time_as_train),
        #     (test_data_rescaled, test_time),
        #     args,
        #     epochs=evaluation_epochs
        # )
        # print('TSTR:Running one step ahead prediction using original+generated data as training set...')
        # real_fake_step_ahead_pred_SMAPE,real_fake_step_ahead_pred_SMAPE_norm,real_fake_step_ahead_pred_training_MSE = one_step_ahead_prediction(
        #     (np.concatenate((train_data_rescaled, generated_data_as_train_rescaled), axis=0), np.concatenate((train_time, generated_time_as_train), axis=0)),
        #     (test_data_rescaled, test_time),
        #     args,
        #     epochs=max(evaluation_epochs//2,1)
        # )
        # one_step_ahead_augmentation_score=np.round(real_step_ahead_pred_SMAPE - min(real_fake_step_ahead_pred_SMAPE,fake_step_ahead_pred_SMAPE), 4)
        # one_step_ahead_augmentation_score_norm=np.round(real_step_ahead_pred_SMAPE_norm - min(real_fake_step_ahead_pred_SMAPE_norm,fake_step_ahead_pred_SMAPE_norm), 4)
        # one_step_ahead_augmentation_score_training_loss=np.round(real_step_ahead_pred_training_MSE - min(real_fake_step_ahead_pred_training_MSE,fake_step_ahead_pred_training_MSE), 4)
        # print('Generated data as training set-one step ahead prediction results(SMAPE):\n' +
        #       f'(1) Real training: {str(np.round(real_step_ahead_pred_SMAPE, 4)),str(np.round(real_step_ahead_pred_SMAPE_norm,4)),str(np.round(real_step_ahead_pred_training_MSE,4))}\n' +
        #       f'(2) Fake training: {str(np.round(fake_step_ahead_pred_SMAPE, 4)),str( np.round(fake_step_ahead_pred_SMAPE_norm,4)),str(np.round(fake_step_ahead_pred_training_MSE,4))}\n' +
        #       f'(3) Real+Fake training: {str(np.round(real_fake_step_ahead_pred_SMAPE, 4)), str(np.round(real_fake_step_ahead_pred_SMAPE_norm,4)),str(np.round(real_fake_step_ahead_pred_training_MSE,4))}\n' +
        ...
        # evaluation_results['one_step_ahead_augmentation_score']=one_step_ahead_augmentation_score
        # evaluation_results['one_step_ahead_augmentation_score_norm']=one_step_ahead_augmentation_score_norm
        # evaluation_results['one_step_ahead_augmentation_score_training_loss']=one_step_ahead_augmentation_score_training_loss
        # evaluation_results_high_level['TSTR:Next-step prediction (SMAPE) improvement']=one_step_ahead_augmentation_score
        # evaluation_results_high_level['TSTR:Next-step norm prediction (SMAPE) improvement ']=one_step_ahead_augmentation_score_norm
        # evaluation_results_high_level['TSTR:Next-step prediction (training loss) improvement']=one_step_ahead_augmentation_score_training_loss
        #
        ...
        # print('high_level_evaluation_results:\n',evaluation_results_high_level,'\n')
    ...
    def baseline_evaluation(self,args,model,result_path,real_data_folder):
            ...
            for experiment_seeds_path in experiment_seeds_paths:
                ...
                for model_path in model_paths:
                    ...
                    if args.prepare_encoded_data:
                        ...
                        generated_data_as_train_rescaled=conditional_rescale_data(generated_train_X, train_scaler_slice, args.differential_features, train_Last_h_data_slice, args.scaler_order, original_feature_order=args.feature)
                        generated_data_as_test_rescaled=conditional_rescale_data(generated_test_X, test_scaler_slice, args.differential_features, test_Last_h_data_slice, args.scaler_order, original_feature_order=args.feature)
                    else:
                    ...
                    #         test_index.append(i)
                    # test_index = np.array(test_index)
                    # train_data_slice = train_X[train_index]
                    # test_data_slice = test_X[test_index]
                    ...
                    # train_H_slice = train_H[train_index]
                    # test_H_slice = test_H[test_index]
                    # print('train_Last_h:',train_Last_h.shape)
                    # print('test_Last_h:',test_Last_h.shape)
                    train_Last_h_slice = train_Last_h[train_index]
                    test_Last_h_slice = test_Last_h[test_index]
                    # print('train_Last_h_slice:',train_Last_h_slice.shape)
                    # print('test_Last_h_slice:',test_Last_h_slice.shape)
        ...
        for result_path in all_result_csv_paths:
            # result_path=f'{args.baseline_real_data_path}/{model_name}_evaluation_results.csv'
            ...
            # df = pd.read_csv(result_path_high_level)
    ...
    def aggregation_baseline_evaluation(self,args,model,result_path,real_data_folder):
        ...
        # model_list = ['TimeGAN', 'RCGAN']
        # model_list = ['SigCWGAN', 'TimeGAN', 'RCGAN']
        # model_list = ['GMMN']
                ...
                for experiment_seeds_path in experiment_seeds_paths:
                    ...
                    for model_path in model_paths:
                        ...
                        if args.prepare_encoded_data:
                            ...
                            # get the train_history_rescaled and test_history_rescaled
                            train_history_rescaled_slice=train_data_rescaled_with_history_slice[:,:args.history_length,:]
