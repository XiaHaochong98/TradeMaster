...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, hidden_dim, d_inner, n_head, dropout=0.1, normalize_before=True):
        ...
        self.pos_ffn = PositionwiseFeedForward(
            hidden_dim, d_inner, dropout=dropout, normalize_before=normalize_before)
...
class Encoder(nn.Module):
    ...
    def __init__(self, configs, window_size, inner_size):
        ...
        d_bottleneck = configs.hidden_dim//4
        ...
        self.layers = nn.ModuleList([
            EncoderLayer(configs.hidden_dim, configs.d_ff, configs.n_heads, dropout=configs.dropout,
                         normalize_before=False) for _ in range(configs.e_layers)
        ])  # naive pyramid attention

        self.enc_embedding = DataEmbedding(
            configs.enc_in, configs.hidden_dim, configs.dropout)
        self.conv_layers = Bottleneck_Construct(
            configs.hidden_dim, window_size, d_bottleneck)
...
class Bottleneck_Construct(nn.Module):
    ...
    def __init__(self, hidden_dim, window_size, d_inner):
        ...
        self.up = Linear(d_inner, hidden_dim)
        self.down = Linear(hidden_dim, d_inner)
