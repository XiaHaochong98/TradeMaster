...
class Feedforward(nn.Module):
    def __init__(self, hidden_dim, dim_feedforward, dropout=0.1, activation='sigmoid'):
        # Implementation of Feedforward model
        super().__init__()
        self.linear1 = nn.Linear(hidden_dim, dim_feedforward, bias=False)
        self.dropout1 = nn.Dropout(dropout)
        self.linear2 = nn.Linear(dim_feedforward, hidden_dim, bias=False)
...
class GrowthLayer(nn.Module):
    ...
    def __init__(self, hidden_dim, nhead, d_head=None, dropout=0.1):
        super().__init__()
        self.d_head = d_head or (hidden_dim // nhead)
        self.hidden_dim = hidden_dim
        self.nhead = nhead

        self.z0 = nn.Parameter(torch.randn(self.nhead, self.d_head))
        self.in_proj = nn.Linear(self.hidden_dim, self.d_head * self.nhead)
        self.es = ExponentialSmoothing(self.d_head, self.nhead, dropout=dropout)
        self.out_proj = nn.Linear(self.d_head * self.nhead, self.hidden_dim)

        assert self.d_head * self.nhead == self.hidden_dim, "hidden_dim must be divisible by nhead"
...
class FourierLayer(nn.Module):
    ...
    def __init__(self, hidden_dim, pred_len, k=None, low_freq=1):
        super().__init__()
        self.hidden_dim = hidden_dim
...
class LevelLayer(nn.Module):
    ...
    def __init__(self, hidden_dim, c_out, dropout=0.1):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.c_out = c_out

        self.es = ExponentialSmoothing(1, self.c_out, dropout=dropout, aux=True)
        self.growth_pred = nn.Linear(self.hidden_dim, self.c_out)
        self.season_pred = nn.Linear(self.hidden_dim, self.c_out)
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, hidden_dim, nhead, c_out, seq_len, pred_len, k, dim_feedforward=None, dropout=0.1,
        ...
        self.growth_layer = GrowthLayer(hidden_dim, nhead, dropout=dropout)
        self.seasonal_layer = FourierLayer(hidden_dim, pred_len, k=k)
        self.level_layer = LevelLayer(hidden_dim, c_out, dropout=dropout)

        # Implementation of Feedforward model
        self.ff = Feedforward(hidden_dim, dim_feedforward, dropout=dropout, activation=activation)
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, hidden_dim, nhead, c_out, pred_len, dropout=0.1):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.nhead = nhead
        self.c_out = c_out
...
class Decoder(nn.Module):
    ...
    def __init__(self, layers):
        super().__init__()
        self.hidden_dim = layers[0].hidden_dim
        self.c_out = layers[0].c_out
        self.pred_len = layers[0].pred_len
        self.nhead = layers[0].nhead

        self.layers = nn.ModuleList(layers)
        self.pred = nn.Linear(self.hidden_dim, self.c_out)
