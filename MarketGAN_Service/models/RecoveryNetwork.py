...
from models.HistoryEmbedding import HistoryEmbeddingNetwork
...
class RecoveryNetwork(torch.nn.Module):
    def __init__(self, args):
        ...
        self.activation_fn_list = [nn.PReLU().to(args.device) if i==low_index else nn.ReLU() for i in range(len(args.feature))]
        self.model =RecoveryRNN(args)
        self.args=args
        # update self.args.condition_dim if add_history
        # make a copy of self.args for self.history_embedding_args
        # self.history_embedding_args=Namespace(**vars(self.args))
        # self.HistoryEmbeddingNetwork=HistoryEmbeddingNetwork(self.history_embedding_args)
        ...
        print(f'RecoveryNetwork use TimesNet: {self.use_TimesNet}, addtional RNN: {self.use_RNN}, add_history:{self.args.add_history}')
