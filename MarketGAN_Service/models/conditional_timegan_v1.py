...
class Conditional_TimeGAN(torch.nn.Module):
    ...
    def __init__(self, args, dynamic_supervisor_args, label_supervisor_args):
        ...
        # TODO: add a sub recovery for each condition(D,L)
        ...
        latent_dynamic_supervisor_args.task_name = 'classification'
        latent_dynamic_supervisor_args.num_class = args.dynamic_dim
        latent_dynamic_supervisor_args.model = args.pretrain_model
        latent_dynamic_supervisor_args.enc_in = args.hidden_dim
        latent_label_supervisor_args = copy.deepcopy(args)
        latent_label_supervisor_args.task_name = 'classification'
        latent_label_supervisor_args.num_class = args.label_dim
        latent_label_supervisor_args.enc_in = args.hidden_dim
        latent_label_supervisor_args.model = args.pretrain_model
        # print('latent_dynamic_supervisor_args',latent_dynamic_supervisor_args)
        self.latent_dynamic_supervisor = Condition_supervisor(latent_dynamic_supervisor_args)
        self.latent_label_supervisor = Condition_supervisor(latent_label_supervisor_args)
    ...
    def _recovery_forward(self, X, T,D=None,L=None,History=None):
        ...
        if D is not None:
            ...
            dynamic_prediction_latent = self.latent_dynamic_supervisor(H)
        ...
        if L is not None:
            ...
            label_prediction_latent = self.latent_label_supervisor(H)
    ...
    def _supervisor_forward(self, X, T,D,L,History=None):
        ...
        # # dynamics loss
        # if D is not None:
        #     pred_D = self.dynamic_supervisor(X)
        ...
        # # Label_loss = torch.nn.functional.mse_loss(train_p, L, reduction='mean')
    ...
    def _generator_forward(self, X, T, Z,D=None,L=None,History=None, gamma=1):
        ...
        if D is not None :
            ...
            D_loss = torch.nn.functional.binary_cross_entropy(D_hat, D)
            Dynamic_hat_s = self.latent_dynamic_supervisor(E_hat)
            D_loss_s = torch.nn.functional.mse_loss(Dynamic_hat_s, D)
        if L is not None:
            # 2. Label
            L_hat = self.label_supervisor(X_hat)
            L_loss =torch.nn.functional.binary_cross_entropy(L_hat, L)
            Label_hat_s = self.latent_label_supervisor(E_hat)
            L_loss_s = torch.nn.functional.mse_loss(Label_hat_s, L)
        if D is not None or L is not None:
            return G_loss,D_loss,L_loss,G_loss_U,G_loss_U_e,G_loss_S,G_loss_V,D_loss_s,L_loss_s
        else:
    ...
    def _pretrain_dynamics_supervisor_foward(self, X, D):
        ...
        #multi-label prediction loss using CrossEntropyLoss
        # Dynamics_loss = torch.nn.functional.mse_loss(train_p, D, reduction='mean')
        # print(type(train_p),type(D))
        # print(train_p.shape,D.shape)
        # print('train_p', train_p.shape, 'D', D.shape)
        Dynamics_loss = torch.nn.functional.binary_cross_entropy(train_p, D)
    ...
    def forward(self, X,obj, T=None, Z=None,D=None,L=None,H=None,gamma=1):
        ...
        else: raise ValueError("`obj` should be either `autoencoder`, `supervisor`, `generator`,`discriminator`,`inference`, `pretrain_dynamic_supervisor`, `pretrain_label_supervisor`")
