...
def embedding_trainer(
    ...
    for epoch in logger:
        for loader in dataloader:
            ...
            if args.conditional:
                _, E_loss0, E_loss_T0,D_loss, L_loss,G_loss_S,Dynamics_loss_latent,Label_loss_latent = model(X=X_mb, T=T_mb, D=D_mb, L=L_mb, H=H_mb, obj="autoencoder")
            else:
            ...
            # Log loss
            if args.conditional:
                logger.set_description(f"Epoch: {epoch}, Loss: {loss:.4f}, D_loss: {D_loss.item():.4f}, L_loss: {L_loss.item():.4f}")
                if writer:
                    writer.add_scalar(
                        "Embedding/Loss:",
                        loss,
    ...
    if args.conditional:
        ...
        for epoch in logger:
            for loader in tqdm(dataloader, position=1, desc="dataloader", leave=False, colour='red',
                ...
                model.zero_grad()
                G_loss, dynmaic_loss_g, label_loss_g,G_loss_U,G_loss_U_e,G_loss_S,G_loss_V,D_loss_latent,L_loss_latent = model(X=X_mb, T=T_mb, Z=Z_mb,D=D_mb,L=L_mb,H=H_mb, obj="generator")
                ...
                # Update model parameters
                g_opt.step()
                logger.set_description( f"Epoch: {epoch}, D_loss_latent: {D_loss_latent:.4f}, L_loss_latent: {L_loss_latent:.4f}, G_loss_V: {G_loss_V:.4f}")
                if writer:
                    writer.add_scalar(
                        'Joint/D_loss_latent:',
                        D_loss_latent,
        ...
        for loader in tqdm(dataloader, position=1, desc="dataloader", leave=False, colour='red',
            ...
            for _ in range(2+addtional_generator_training):
                ...
                if args.conditional:
                    ...
                    G_loss,G_loss_U,G_loss_U_e,G_loss_S,G_loss_V = model(X=X_mb, T=T_mb, Z=Z_mb,D=D_mb,L=L_mb,H=H_mb, obj="generator")
                if args.conditional:
                ...
                # Update model parameters
                g_opt.step()
                # TODO: why we need to update supervisor parameters here?
                s_opt.step()

                # Forward Pass (Embedding)
                model.zero_grad()
                if args.conditional:
                    E_loss, _, E_loss_T0,dynmaic_loss_e, label_loss_e,G_loss_S,Dynamics_loss_latent,Label_loss_latent= model(X=X_mb, T=T_mb,D=D_mb,L=L_mb,H=H_mb, obj="autoencoder")
            ...
            # Forward Pass
            if args.conditional:
                D_loss,D_loss_real,D_loss_fake,D_loss_fake_e = model(X=X_mb, T=T_mb, Z=Z_mb,D=D_mb,L=L_mb,H=H_mb, obj="discriminator")
            else:
                D_loss,D_loss_real,D_loss_fake,D_loss_fake_e = model(X=X_mb, T=T_mb, Z=Z_mb,D=D_mb,L=L_mb,H=H_mb, obj="discriminator")
...
def supervisors_pretain_trainer(
    ...
    for epoch in logger:
        for X_mb, T_mb, D_mb, L_mb,H_mb in tqdm(dataloader, position=1, desc="dataloader", leave=False, colour='red', ncols=80):
            # Reset gradients
            model.zero_grad()
            # print('forward')
            # Forward Pass
            ...
            # Log loss
            logger.set_description(f"Epoch: {epoch}, Dynamics_loss: {d_loss:.4f},Dynamics_acc:{d_acc:.4f}, Label_loss: {l_loss:.4f},Label_acc:{l_acc:.4f}")
            if writer:
                writer.add_scalar(
...
def conditional_timegan_trainer(model, data, time, args,dynamics=None,labels=None,history=None):
    ...
    conditional_dataset = ConditionalTimeGANDataset(data=data,time=time,dynamic=dynamics,label=labels,history=history)
...
def conditional_timegan_condition_prediction(model, X_mb,H_mb,D_mb,L_mb, args):
    ...
    with torch.no_grad():
        ...
        d_loss_list = []
        l_loss_list = []
        for X_mb, T_mb, D_mb, L_mb,H_mb in tqdm(conditional_dataloader, position=1, desc="dataloader", leave=False, colour='red',
                                           ncols=80):
            dynamic_loss, d_acc = model(X=X_mb, D=D_mb, obj="pretrain_dynamic_supervisor")
            label_loss, l_acc = model(X=X_mb, L=L_mb, obj="pretrain_label_supervisor")
    ...
    return sum(d_acc_list)/len(d_acc_list), sum(l_acc_list)/len(l_acc_list), sum(d_loss_list)/len(d_loss_list), sum(l_loss_list)/len(l_loss_list)

def conditional_timegan_generator(model, T, args,dynamics=None,labels=None,history=None,seed=None,noise_multiplier=1):
    ...
    # Initialize TimeGAN dataset and dataloader
    if not seed:
        Z = torch.rand((len(T), args.max_seq_len, args.Z_dim))*noise_multiplier
    else:
        # print('seed',seed,type(seed))
        # set torch seed
        torch.manual_seed(seed)
        Z = torch.rand((len(T), args.max_seq_len, args.Z_dim),generator=torch.Generator().manual_seed(seed))*noise_multiplier

    conditional_dataset = ConditionalTimeGANDataset(data=Z,time=T,dynamic=dynamics,label=labels,history=history)
    # print all attributes of the dataset
    # print('H',conditional_dataset.H)
    # print(f'generation batch_size is {batch_size}, noise multiplier is {noise_multiplier}')
    conditional_dataloader = torch.utils.data.DataLoader(
        dataset=conditional_dataset,
...
def conditional_timegan_generator_samples(model, T, args,dynamics=None,labels=None,history=None,seed_list=None,noise_multiplier=1):
    """The inference procedure for TimeGAN
    Args:
        - model (torch.nn.module): The model that generates synthetic data
        - T (List[int]): The time to be generated on
    ...
    if not seed_list:
        ...
        for seed in seed_list:
            Z_sample = torch.rand((len(T), args.max_seq_len, args.Z_dim),generator=torch.Generator().manual_seed(seed))*noise_multiplier
            Z_list.append(Z_sample)
        # conatenate the Z_list to Z
        Z = torch.cat(Z_list,dim=0)
        # expand the T,dynamics,labels,history to match the Z
        T = T*len(seed_list)
        if dynamics:
            dynamics = dynamics*len(seed_list)
        if labels:
            labels=labels*len(seed_list)
        if history:
